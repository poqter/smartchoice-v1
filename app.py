...
if st.button("결과 보기"):
    if deposit_monthly in (None, 0) or deposit_rate in (None, 0.0) or insurance_monthly in (None, 0) or return_rate in (None, 0.0):
        ...
        with colm1:
            st.metric("세후 이자 총합 (10년 기준)", <span style='color:red; font-weight:bold;'>f"{int(total_after_tax_interest_10y // 1)}만원</span>)
        ...
        with colm2:
            st.metric("단기납 보너스 총합 (10년 기준)", <span style='color:red; font-weight:bold;'>f"{int(bonus // 1)}만원", delta=f"{bonus - total_after_tax_interest_10y:,.0f}만원</span>")
