...
if st.button("결과 보기"):
    if deposit_monthly in (None, 0) or deposit_rate in (None, 0.0) or insurance_monthly in (None, 0) or return_rate in (None, 0.0):
        ...
        with colm2:
            st.markdown(f"<div style='font-size:18px; font-weight:bold;'>단기납 보너스 총합 (10년 기준): <span style='color:red;'>{int(bonus)}만원</span> <span style='font-size:14px; color:gray;'>(+{bonus - total_after_tax_interest_10y:,.0f}만원)</span></div>", unsafe_allow_html=True)
